...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", CryptoCom: CryptoComClient,  *args, **kwargs):
        ...
        self._exchanges = {"CryptoCom": CryptoCom}
        ...
        for exchange, client in self._exchanges.items():
            for symbol, contract in client.contracts.items():
                # If you want less symbols in the list, filter here (there are a lot of pairs on CryptoCom Spot)
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name + "_var"][b_index].set(base_param['values'])
